...
None. E5.9b is the latest shipped sub-deliverable. Peak refinement
...
finding (live corpus growth → ~7% chunk-count drift, −2 to −4 pt
score drift on the same geometry across a 2-day window). The
E5.7 global-default flip (bge-base → e5-base, commit `00e3fd3`)
is not affected — the flip evidence was single-snapshot. Remaining
E5.9 work (nomic peak refinement, then cross-corpus validation)
continues per the "Next" section below.
