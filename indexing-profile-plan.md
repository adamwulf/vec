...
| ✅ DONE | 3c-r | 2 reviewer agents in parallel | `review-cycle` on Phase 3c → manager approval → next phase | 30–60 min |
...
| ⏳ NEXT UP | 3e | one impl agent | Wire `InsertCommand` + `SearchCommand` + `InfoCommand` + `ListCommand` + `ResetCommand`; delete `EmbedderFactory`; drop `RecursiveCharacterSplitter` defaults; design doc | 1.5–2 h |
