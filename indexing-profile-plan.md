...
4. `--embedder nomic` / `--embedder nl` selects a built-in profile with
   its defaults. `--chunk-chars N` / `--chunk-overlap M` override on top
   of the selected profile; any override creates a derived profile
   with identity `alias@N/M` (e.g. `nomic@500/100`). No override means
   the canonical identity is used unchanged.
...
10. Test coverage:
    - `IndexingProfileTests` — identity construction, override
      composition, round-trip through the DB config.
    - `DatabaseConfigTests` — new profile shape round-trip; pre-profile
      DB JSON decodes to `profile == nil` so `readConfig` itself
      doesn't crash (the subsequent "operation requires recorded
      profile" hard-fail belongs to each command).
    - `TrademarkTranscriptFixtureTests` extended: a single parameterized
      test loops over `IndexingProfileFactory.allBuiltInProfiles` and
      verifies extraction + embed for every profile. New profile → new
      coverage for free.
...
public struct IndexingProfile: Sendable {
    ...
    /// CLI overrides. Purely informational — used by `info`/error text
    /// to render "nomic (default)" vs "nomic (custom)".
...
public enum IndexingProfileFactory {
    ...
    /// Maps a canonical embedder name (as persisted) back to its alias,
    /// or nil for an unknown embedder (future/retired).
    public static func alias(forCanonicalEmbedderName canonical: String) -> String? {
        builtIns.first(where: { $0.canonicalEmbedderName == canonical })?.alias
    }

    /// Constructs a live profile from an alias + optional overrides.
    ...
    /// live profile. Throws `VecError.unknownProfile(identity)` on
    /// a malformed identity or an unknown alias portion.
    public static func resolve(identity: String) throws -> IndexingProfile {
        // Expected shape: "<alias>@<size>/<overlap>"
        guard let atIdx = identity.firstIndex(of: "@") else {
            throw VecError.unknownProfile(identity)
        }
        let alias = String(identity[..<atIdx])
        let rest = identity[identity.index(after: atIdx)...]
        let parts = rest.split(separator: "/", maxSplits: 1, omittingEmptySubsequences: false)
        guard parts.count == 2,
              let size = Int(parts[0]),
              let overlap = Int(parts[1]) else {
            throw VecError.unknownProfile(identity)
        }
...
`profile == nil` (the key simply isn't present) which every command
turns into a hard fatal `VecError.profileNotRecorded` telling the
user to `vec reset`.
...
`VecError.profileNotRecorded` (search/insert on a fresh or pre-
profile DB):

```
Database has no recorded indexing profile (fresh, reset, or pre-profile).
Run `vec update-index --embedder <alias>` first to establish a profile.
```

`VecError.unknownProfile(identity)` (future-version DB or malformed
identity):

```
Unknown or unparseable indexing profile '<identity>'. This build
knows: nomic@<dflt>/<dflt>, nl@<dflt>/<dflt>. The database may
have been indexed by a newer version of vec, or the config.json
may be corrupt. Run `vec reset <db>` to rebuild.
```

`VecError.invalidChunkParams(detail)` (precondition failure from
the factory):
...
vec update-index
# subsequent index: resolves profile from recorded identity, ignores
# (but refuses) any override that would change identity

vec update-index --embedder nl
# recorded is "nomic@1200/240" → HARD FAIL with profileMismatch

vec update-index --chunk-chars 500
# recorded is "nomic@1200/240" → HARD FAIL: recorded identity is
# "nomic@1200/240", requested is "nomic@500/240" (overlap carried
# over as unspecified → takes built-in default for resolution,
# which is 240; still a mismatch because size differs).
...
vec info
# prints "Profile: nomic@1200/240 (768d)" or "(not yet recorded)".
```

**Rule on partial overrides:** if the user passes one of
`--chunk-chars` / `--chunk-overlap` without the other, the missing
one resolves to the alias's built-in default. This is documented
explicitly: partial override + built-in-default-for-the-other is a
deliberate choice so `vec update-index --chunk-chars 500` is a
terse way to say "nomic with 500-char chunks and whatever overlap
nomic defaults to." The resulting identity is always fully concrete.
...
1. Parse CLI flags: alias, size, overlap. Validate chunk params.
2. `DatabaseLocator.resolve(name)` → raw config.
3. Open the DB **only to count chunks**. This is the point where
   pre-profile DBs trip: their `profile == nil` *and* `chunkCount > 0`
   → raise `profileNotRecorded` immediately with the reset message.
   Fresh/reset DBs (`profile == nil` *and* `chunkCount == 0`) fall
   through to the first-index branch.
4. If recorded: construct requested identity from CLI flags (alias
   falls back to the recorded alias if `--embedder` omitted, same
   for chunk params). Compare requested identity against
   `recorded.identity`. Mismatch → `profileMismatch`.
5. If recorded and matching, or not recorded: call
   `IndexingProfileFactory.make` / `resolve` to produce the live
   profile.
6. If not recorded: persist the `ProfileRecord` to config.json
   before the pipeline touches the DB.
7. Build `VectorDatabase(dimension: profile.embedder.dimension)` and
   `IndexingPipeline(profile:)`. Run.

For `search` and `insert`:

1. Resolve DB. Open the DB (for chunk count if needed — search
   doesn't strictly need it, but insert does).
2. If `config.profile == nil` → `profileNotRecorded`.
3. `IndexingProfileFactory.resolve(identity: recorded.identity)` →
   live profile. An unknown alias portion throws `unknownProfile`.
...
- `Sources/VecKit/FileScanner.swift` (VecError home) — rename
  ...
  add `invalidChunkParams(String)`. `dimensionMismatch` stays
  (belt-and-braces).
...
- `Sources/vec/Commands/ResetCommand.swift` — write config with
  `profile: nil`.
- `Sources/vec/Commands/InfoCommand.swift` — print `Profile:
  <identity> (<dim>d)` or `(not yet recorded)`. Remove
...
1. `IndexingProfileTests` — unit tests, no model load:
   ...
   - Override: `make(alias: "nomic", chunkSize: 500, chunkOverlap: 100)`
     → identity `"nomic@500/100"`, `isBuiltIn == false`,
     `splitter.chunkSize == 500`.
   - Partial override: `make(alias: "nomic", chunkSize: 500, chunkOverlap: nil)`
     → `"nomic@500/240"` (overlap falls to default).
   - `resolve(identity: "nomic@1200/240")` → matches `make(alias: "nomic")`.
   - `resolve(identity: "garbled")` → `VecError.unknownProfile`.
   - `resolve(identity: "nomic@abc/def")` → `VecError.unknownProfile`.
   ...
   - `make(chunkSize: -1, …)` → `VecError.invalidChunkParams`.
...
2. Rename `EmbedderConfigTests` → `IndexingProfileConfigTests`:
   ...
   - Decode legacy JSON (pre-profile shape — may still carry the old
     ...
     decode-only.
   - Factory alias round-trip (`alias(forCanonicalEmbedderName:)`
     returns the alias).
   - Unknown alias error.
   - Default alias is in `knownAliases`.

3. `TrademarkTranscriptFixtureTests` — restructure:
   ```swift
...
Total test count delta: +~10 (new IndexingProfileTests), -2 (the
two hardcoded fixture tests collapse into one loop), net ~+8. Full
`swift test` must stay green at every commit.
...
| 3 | one impl agent | Code + `indexing-profile.md` design doc | 3–4 h |
...
Phase 3 is the single longest chunk. Rough yardstick from the
pluggable-embedders impl (`~3-4 h`) applies directly: the shape is
the same refactor — protocol-plus-factory-plus-config-plus-CLI —
with a similar count of touched files. The actual code work is
smaller (no new model integration) but the CLI flag plumbing and
check-order is more intricate, so the two roughly cancel.
...
1. **`IndexingProfile` struct + `IndexingProfileFactory`** —
   ...
   `IndexingProfileTests`. 179 tests + 10 new = 189 pass.
2. **`VecError` rename wave** — rename `unknownEmbedder` →
   `unknownProfile` etc., add `invalidChunkParams`. Mechanical;
   every call-site flips in one commit.
...
7. **`TrademarkTranscriptFixtureTests` loopification** — last, so
   it benefits from the new factory surface.
8. **Delete `Sources/VecKit/EmbedderFactory.swift`** — verify no
   remaining references.
9. **Write `indexing-profile.md`** — the design doc.
...
- [ ] `IndexingProfileTests` covers: built-in resolution (both
      aliases), override composition (both partial and full),
      identity parsing (good + bad), invalid-chunk-param errors,
...
2. **Partial override + built-in default collision.** If someone
   runs `--embedder nomic --chunk-chars 1200` (explicitly supplying
   nomic's default), the identity is `"nomic@1200/240"` — the
   same as the built-in. Fine: the `isBuiltIn` flag distinguishes
   "user typed it" from "factory picked it," but the persisted
   identity is the same and the mismatch check compares on
   identity alone. An override that coincides with the default
   is indistinguishable from "took the default" post-persist,
